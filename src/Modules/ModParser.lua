...
local specialModList = {
	...
	["increases and reductions to minion cast speed also affects? you"] = { flag("MinionCastSpeedAppliesToPlayer"), mod("ImprovedMinionCastSpeedAppliesToPlayer", "MAX", 100) },
	...
	["(%d+)%% chance to gain an? (%w+) charge on critical strike"] = function(num, _, chargeType) return { mod(chargeType:gsub("^%l", string.upper) .. "ChargeGainOnCrit", "BASE", num) } end,
	...
	["(%d+)%% chance to gain a power charge on critical strike with wands"] = function(num) return { mod("PowerChargeGainOnCrit", "BASE", num,{ type = "Condition", var = "UsingWand" })} end,
	...
	["gain a frenzy charge on critical strike"] = { mod("FrenzyChargeGainOnCrit", "BASE", 100 ) },
